...
//全局坐标转换为汽车坐标
Eigen::MatrixXd transformCoordinatesToVehicle(double x, double y, double psi, const vector<double> &ptsx,
        ...
        if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
            ...
            if (s != "") {
                ...
                if (event == "telemetry") {
                    ...
                    //转换坐标体系，从全局左边变为汽车坐标
                    Eigen::MatrixXd waypoints = transformCoordinatesToVehicle(px, py, psi, ptsx, ptsy);
                    Eigen::VectorXd Ptsx = waypoints.row(0);
                    Eigen::VectorXd Ptsy = waypoints.row(1);

                    //拟合目标路径
                    Eigen::VectorXd fit_curve = polyfit(Ptsx, Ptsy, 3);
                    ...
                    mpc.Solve(state, fit_curve);
                    double steer_value = mpc.predict_delta;
                    double throttle_value= mpc.predict_a;
